...
export default function Chat() {
  ...
  // Salva a sessão quando recebida da API
  ...
  const sendMessage = async (text) => {
    ...
    try {
      ...
      // Verifica se a resposta trouxe um novo session_id
      ...
      const errorMsg = err.response?.data?.detail || 'Erro ao conectar com a IA';
  ...
  const handleAudioSend = async (formData) => {
    ...
    try {
      ...
      const errorMsg = err.response?.data?.detail || 'Erro ao processar áudio';
      toast.error(errorMsg);
      setMessages(prev => [...prev, { text: `❌ ${errorMsg}`, isUser: false }]);
    } finally {
      setLoading(false);
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', height: 'calc(100vh - 180px)' }}>
      ...
      <p style={{ color: '#555', fontSize: '0.75rem', marginTop: '10px', textAlign: 'center' }}>
        ...
        <a href={API_URL} target="_blank" rel="noopener noreferrer" style={{ color: '#d4af37', marginLeft: '6px' }}>
          Ver status da API
        </a>
      </p>
    </div>
